...
func TestSM2PrivateKey(t *testing.T) {
	...
	var buf bytes.Buffer
	if err := NewSM2PrivateKey(time.Now(), sm2Priv).Serialize(&buf); err != nil {
		t.Fatal(err)
	}
	//fmt.Printf("%x", sm2Priv.oid)
...
func TestSM2SignerPrivateKey(t *testing.T) {
	...
	priv := NewSignerPrivateKey(time.Now(), &sm2Signer{sm2Priv})

	if priv.PubKeyAlgo != PubKeyAlgoSM2 {
		t.Fatal("NewSignerPrivateKey should have made an SM2 private key")
	}
